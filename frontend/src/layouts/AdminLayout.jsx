...
const AdminLayout = () => {
  ...
  const navLinks = [
    ...
    {
      label: 'All Assessments', path: '/admin/assessments',
    ...
    },
    {
      ...
      icon: <svg className="w-5 h-5 mr-3" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M13 10V3L4 14h7v7l9-11h-7z" /></svg>
    },
    {
      label: 'Analytics', path: '/admin/analytics',
      icon: <svg className="w-5 h-5 mr-3" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M16 8v8m-4-5v5m-4-2v2m-2 4h12a2 2 0 002-2V6a2 2 0 00-2-2H6a2 2 0 00-2 2v12a2 2 0 002 2z" /></svg>
    },
  ];
      ...
      <aside className="w-64 bg-white dark:bg-gray-800 border-r border-gray-200 dark:border-gray-700 hidden md:flex flex-col justify-between">
        <div>
          ...
          <div className="p-4 flex items-center border-b border-gray-100 dark:border-gray-700/50">
            <div className="w-9 h-9 rounded-full bg-gradient-to-tr from-red-500 to-orange-400 flex items-center justify-center text-white font-bold text-sm mr-3">
              {user?.name?.charAt(0) || 'A'}
            </div>
            <div>
              <p className="text-sm font-semibold text-gray-900 dark:text-white truncate max-w-[130px]">{user?.name || 'Admin'}</p>
        ...
        <div className="p-3 border-t border-gray-200 dark:border-gray-700 space-y-1">
          <Link to="/admin/settings"
            className={`flex items-center px-4 py-2.5 w-full text-sm font-medium rounded-xl transition-colors ${
      ...
      <main className="flex-1 flex flex-col h-screen overflow-hidden">
        <header className="h-16 bg-white dark:bg-gray-800 border-b border-gray-200 dark:border-gray-700 flex items-center justify-between px-6 shrink-0">
          ...
          <div className="flex items-center space-x-3 ml-4">
            ...
            <Link to="/admin/profile" className="flex items-center space-x-2.5 pl-3 border-l border-gray-200 dark:border-gray-700 hover:opacity-80 transition-opacity">
              <div className="w-8 h-8 rounded-full bg-gradient-to-tr from-red-500 to-orange-400 flex items-center justify-center text-white font-bold text-sm">
                {user?.name?.charAt(0) || 'A'}
              </div>
              <span className="text-sm font-semibold text-gray-700 dark:text-gray-200 hidden sm:block">{user?.name?.split(' ')[0] || 'Admin'}</span>
            </Link>
          </div>
        </header>
